...
// .wrangler/tmp/bundle-gBzlxD/checked-fetch.js
...
async function handleDailyQuiz(url, supabase) {
  ...
  let query = supabase.from("quizzes").select("id, title, description, play_count, rank, correct_rate, incorrect_rate, game_count").limit(100);
  if (sort === "rank") {
...
async function handleWorldcups(supabase) {
  const { data, error } = await supabase.from("worldcups").select("id, title, description, play_count").order("created_at", { ascending: false }).limit(100);
  if (error) return errorResponse(error.message);
  return jsonResponse(data);
}
...
// .wrangler/tmp/bundle-gBzlxD/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gBzlxD/middleware-loader.entry.ts
