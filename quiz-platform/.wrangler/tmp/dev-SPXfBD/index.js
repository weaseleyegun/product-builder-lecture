...
async function handleDailyQuiz(supabase) {
  const { data, error } = await supabase.from("quizzes").select("id, title, description").order("created_at", { ascending: false }).limit(5);
...
async function handleWorldcups(supabase) {
  const { data, error } = await supabase.from("worldcups").select("id, title, description, play_count").order("created_at", { ascending: false }).limit(5);
  if (error) return errorResponse(error.message);
  return jsonResponse(data);
}
...
async function handleUserCreatedContent(request, supabase) {
  try {
    ...
    const { data: cupData, error: cupError } = await supabase.from("worldcups").insert([{ title, description }]).select();
    if (cupError) {
