...
// .wrangler/tmp/bundle-Fll69N/checked-fetch.js
...
async function handleDailyQuiz(url, supabase) {
  ...
  let query = supabase.from("quizzes").select("id, slug, title, description, thumbnail_url, play_count, rank, correct_rate, incorrect_rate, game_count, creator_id").limit(100);
  if (sort === "rank") {
...
__name(handleDailyQuiz, "handleDailyQuiz");
...
__name(handleQuizPlay, "handleQuizPlay");
async function handleQuizResult(request, supabase) {
  try {
    ...
    const { data: quiz, error: fetchErr } = await supabase.from("quizzes").select("correct_count, incorrect_count, game_count").eq("id", quizId).single();
    ...
    const correct_rate = total > 0 ? Math.round(newCorrect / total * 1e4) / 100 : 0;
    const incorrect_rate = total > 0 ? Math.round(newIncorrect / total * 1e4) / 100 : 0;
    if (rpcErr) {
    ...
    const { data: allQuizzes } = await supabase.from("quizzes").select("id, play_count").order("play_count", { ascending: false });
    if (allQuizzes) {
      for (let i = 0; i < allQuizzes.length; i++) {
        await supabase.from("quizzes").update({ rank: i + 1 }).eq("id", allQuizzes[i].id);
      }
    }
...
__name(handleQuizResult, "handleQuizResult");
async function handleUserCreatedQuiz(request, supabase) {
  try {
    ...
    const { data: quizData, error: quizError } = await supabase.from("quizzes").insert([{ title, description: description || "", creator_id: creatorId }]).select();
    if (quizError) {
...
__name(handleUserCreatedQuiz, "handleUserCreatedQuiz");

// src/routes/worldcup.js
async function handleWorldcups(url, supabase) {
  const sort = url.searchParams.get("sort") || "rank";
  let query = supabase.from("worldcups").select("id, slug, title, description, thumbnail_url, play_count, created_at, creator_id").limit(100);
  if (sort === "rank") {
    query = query.order("play_count", { ascending: false });
  } else if (sort === "latest") {
    query = query.order("created_at", { ascending: false });
...
__name(handleWorldcupPlay, "handleWorldcupPlay");
async function handleUserCreatedContent(request, supabase) {
  try {
    ...
    const { data: cupData, error: cupError } = await supabase.from("worldcups").insert([{ title, description: description || "", creator_id: creatorId }]).select();
...
async function verifyOwnerOrAdminToken(request, tableName, itemId, supabase, env) {
  ...
  const { data: itemData, error: itemError } = await supabase.from(tableName).select("creator_id").eq("id", itemId).single();
...
__name(verifyOwnerOrAdminToken, "verifyOwnerOrAdminToken");
async function handleUpdateQuiz(request, url, supabase, env) {
  ...
  try {
    ...
    const { error: updateErr } = await supabase.from("quizzes").update({ title, description, thumbnail_url }).eq("id", id);
    ...
    return errorResponse(err2.message || "Internal Server Error", 500);
  }
}
__name(handleUpdateQuiz, "handleUpdateQuiz");
async function handleUpdateWorldcup(request, url, supabase, env) {
  ...
  try {
    ...
    const { error: updateErr } = await supabase.from("worldcups").update({ title, description, thumbnail_url }).eq("id", id);
    if (updateErr) return errorResponse("DB Update Error: " + updateErr.message, 500);
...
__name(handleUpdateWorldcup, "handleUpdateWorldcup");
async function handleDeleteQuiz(request, url, supabase, env) {
  ...
  try {
    ...
    return errorResponse(err2.message, 500);
  }
}
__name(handleDeleteQuiz, "handleDeleteQuiz");
...
// .wrangler/tmp/bundle-Fll69N/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Fll69N/middleware-loader.entry.ts
