...
// .wrangler/tmp/bundle-31RzFC/checked-fetch.js
...
async function handleUserCreatedQuiz(request, supabase) {
  try {
    ...
    const { data: quizData, error: quizError } = await supabase.from("quizzes").insert([{ title, description: description || "", is_user_created: true }]).select();
    if (quizError) {
...
// .wrangler/tmp/bundle-31RzFC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-31RzFC/middleware-loader.entry.ts
