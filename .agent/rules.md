...
## 7. 🤖 에이전트 팀 구성 (Agent Team)

### 팀 구성도

```
┌─────────────────────────────────────────────────┐
│              🎯 Agent Team (QuizRank.io)        │
├──────────┬──────────────────────────────────────┤
│ 단계     │ 에이전트                              │
├──────────┼──────────────────────────────────────┤
│ 🧠 기획  │ /planning  — 아이디어 → 구현 설계     │
├──────────┼──────────────────────────────────────┤
│ 🛠 실행  │ (기본 에이전트가 코드 작성)            │
├──────────┼──────────────────────────────────────┤
│ 🧪 검증  │ /qa       — 브라우저 QA 검증          │
│          │ /health   — DB 건강검진 + 오염 청소    │
├──────────┼──────────────────────────────────────┤
│ 🚀 배포  │ /git      — 커밋 & 푸시               │
│          │ /deploy   — 배포 & 라이브 확인         │
├──────────┼──────────────────────────────────────┤
│ 📝 기록  │ /summary  — progress.md 갱신          │
└──────────┴──────────────────────────────────────┘
```
...
```
/planning → 코드 작성 → /qa → /git → /deploy → /summary
```
...
- **배포 전 검증**: 프로덕션 배포 전에는 `/qa`로 주요 페이지를 점검한다.
- **정기 건강검진**: 주 1회 이상 `/health`로 DB 무결성을 점검한다.
- **커밋 단위**: 하나의 기능 또는 이슈 단위로 `/git`을 실행하여 작은 커밋을 유지한다.
